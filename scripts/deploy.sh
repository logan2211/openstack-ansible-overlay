...
if [[ "$UPDATE_ANSIBLE" == "yes" ]] || [[ ! $(which openstack-ansible) ]]; then
  scripts/bootstrap-ansible.sh

  # Update Overlay roles
  if [ -f "${OVERLAY_DIR}/../ansible-role-requirements.yml" ];then
      # Pull all required roles.
      ansible-galaxy install --role-file="${OVERLAY_DIR}/../ansible-role-requirements.yml" \
                             --ignore-errors \
                             --force
  fi
fi
